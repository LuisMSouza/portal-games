...
async function Details() {
    ...
    let publisherId = parseInt(urlParams.get('id'))

    if (!publisherId) return;

    loadInfos(publisherId);
}

async function loadInfos(publisherId){
  try{
    const result = await fetch(`https://api.rawg.io/api/publishers/${publisherId}?key=${Key}`);
